...
//
//   npm run test:ui -- <name> [--shot='<js to run before the screenshot>']
//
...
if (!name) {
  console.error("usage: npm run test:ui -- <check-name> [--shot='<js>']");
  process.exit(2);
}
...
try {
  ...
  console.log("screenshot:", join("test/ui/out", name + ".png"));
} finally {
  await browser.close();
  server.close();
}
